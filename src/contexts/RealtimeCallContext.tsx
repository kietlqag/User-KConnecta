...
export function RealtimeCallProvider({ children }: { children: ReactNode }) {
  ...
  const maybeShowDesktopNotification = useCallback(
    async (signal: IncomingCallSignal) => {
      ...
      let callerName = peerProfiles[signal.fromUserId]?.fullName || signal.fromUsername || 'Ngý?i dùng';
      if (!peerProfiles[signal.fromUserId] && !peerProfileLoadingRef.current.has(signal.fromUserId)) {
        ...
        try {
          ...
          const resolvedName = profile.fullName || profile.username || 'Ngý?i dùng';
      ...
      const notif = new Notification(
        signal.mediaType === 'video' ? 'Cu?c g?i video ð?n' : 'Cu?c g?i tho?i ð?n',
        {
          body: `${callerName} ðang g?i cho b?n`,
      ...
      void (async () => {
        try {
          ...
          setPeerProfiles((prev) => ({
            ...prev,
            [peerId]: {
              fullName: profile.fullName || profile.username || 'Ngý?i dùng',
              avatarUrl: profile.avatarUrl,
            },
          }));
        } catch {
    ...
    const syncSession = async () => {
      try {
        ...
        if (snapshot.status === 'RINGING') {
          setAuthoritativeStatusText('Ðang g?i...');
          return;
        }
  ...
  const incomingName = incomingProfile?.fullName || voiceCall.incomingFromUsername || 'Ngý?i dùng';
  const incomingAvatar =
    incomingProfile?.avatarUrl ||
    `https://ui-avatars.com/api/?name=${encodeURIComponent(incomingName)}&background=random`;
  const activeName =
    activeProfile?.fullName || voiceCall.activePeerDisplayName || voiceCall.incomingFromUsername || 'Ngý?i dùng';
  const activeAvatar =
  ...
  const incomingCallText =
    voiceCall.incomingMediaType === 'video' ? 'Ðang có cu?c g?i video ð?n...' : 'Ðang có cu?c g?i ð?n...';
