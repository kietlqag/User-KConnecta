  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[640px] gap-0 overflow-hidden rounded-[20px] border-0 p-0 sm:max-h-[72vh]">
        <DialogHeader className="border-b border-border px-5 pt-4">
          ...
          <div className="flex items-center gap-5 pr-10 relative">
            <button
              ...
              className={cn( 'border-b-[3px] pb-3 text-[16px] font-semibold transition-colors', activeFilter === 'ALL' ? 'border-emerald-500 text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground', )}
            >
              Tất cả
            </button>
            ...
            {activeReactions.slice(0, 3).map((reaction) => (
              <button
                ...
                className={cn( 'flex items-center gap-1.5 border-b-[3px] pb-3 text-[16px] font-semibold transition-colors', activeFilter === reaction.type ? 'border-emerald-500 text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground', )}
              >
            ...
            {activeReactions.length > 3 && (
              <button
                ...
                className="flex items-center gap-1 border-b-[3px] border-transparent pb-3 text-[16px] font-semibold text-muted-foreground hover:text-foreground"
              >
                <span>Xem thêm</span>
            ...
            {activeReactions.length > 3 && showMoreOpen && (
              <div className="absolute left-[120px] top-[52px] z-20 min-w-[160px] rounded-xl border border-border bg-card py-1 shadow-lg">
                {activeReactions.slice(3).map((reaction) => (
                  <button
                    ...
                    className={cn( 'flex w-full items-center gap-2 px-3 py-2 text-left text-[14px]', activeFilter === reaction.type ? 'bg-emerald-50 font-semibold text-foreground' : 'text-foreground hover:bg-muted', )}
                  >
                    ...
                    <span className="text-[13px] text-muted-foreground">{resolvedCounts[reaction.type]}</span>
                  </button>
                ))}
              </div>
            )}
        ...
        <div className="max-h-[480px] overflow-y-auto px-5 py-3.5">
          {isLoading && <p className="text-sm text-muted-foreground">Đang tải danh sách cảm xúc...</p>}

          {!isLoading && error && <p className="text-sm text-red-500">{error}</p>}

          {!isLoading && !error && totalCount <= 0 && (
            <p className="text-sm text-muted-foreground">Chưa có cảm xúc nào cho bài viết này.</p>
          )}
                  ...
                  <div key={`${user.userId}-${user.reactionType}`} className="flex items-center justify-between gap-3">
                    <div className="flex min-w-0 items-center gap-3">
                      <div className="relative h-12 w-12 shrink-0">
                        ...
                        <span className="absolute bottom-0 right-0 flex h-5 w-5 items-center justify-center rounded-full border-2 border-white bg-card leading-none">
                          {reaction?.emoji && <img src={reaction.emoji} alt={reaction.label} width={14} height={14} draggable={false} />}
                        </span>
                      </div>

                      <div className="min-w-0">
                        <p className="truncate text-[14px] font-semibold text-foreground">{user.fullName}</p>
                    ...
                    <button
                      type="button"
                      className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-muted px-4 py-2.5 text-[14px] font-semibold text-foreground transition-colors hover:bg-muted"
                    >
