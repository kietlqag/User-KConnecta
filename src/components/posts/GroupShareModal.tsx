...
import { Link2, MessageCircle, Users, ArrowLeft, Search, Globe, Lock, Smile, ChevronDown } from 'lucide-react';
...
export function GroupShareModal({
  ...
  const buildMessengerShareContent = () =>
    `${GROUP_SHARE_PREFIX}${JSON.stringify({
      id: groupId,
      name: groupName,
      cover: groupCoverUrl || undefined,
  ...
  const handleShareToFeed = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Không thể chia sẻ nhóm');
    } finally {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) handleClose(); }}>
      <DialogContent className="sm:max-w-[500px] p-0 overflow-visible bg-white dark:bg-gray-800 rounded-2xl border-none shadow-2xl">
        <DialogDescription className="sr-only">Chia sẻ nhóm lên bảng tin hoặc gửi cho bạn bè</DialogDescription>
        {showFriendPicker ? (
          <>
          ...
          <>
            <DialogHeader className="p-4 border-b">
              <DialogTitle className="text-center text-xl font-bold">Chia sẻ nhóm</DialogTitle>
            </DialogHeader>
            ...
            <div className="p-4 flex flex-col gap-4">
              ...
              <div className="relative">
                <textarea
                  ...
                  placeholder="Hãy nói gì đó về nhóm này..."
                  rows={3}
                  maxLength={1000}
              ...
              <div className="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                {groupCoverUrl ? (
                  <img src={groupCoverUrl} alt={groupName} className="h-28 w-full object-cover" />
                ) : (
                ...
                <div className="p-3">
                  <p className="truncate text-sm font-semibold text-gray-900 dark:text-gray-100">{groupName}</p>
                  <div className="mt-0.5 flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
                    {groupPrivacy === 'PUBLIC' ? <Globe className="h-3.5 w-3.5" /> : <Lock className="h-3.5 w-3.5" />}
                    <span>{groupPrivacy === 'PUBLIC' ? 'Nhóm Công khai' : 'Nhóm Riêng tư'}</span>
                    <span aria-hidden>·</span>
                    <span>{groupMemberCount.toLocaleString('vi-VN')} thành viên</span>
                  </div>
                </div>
              </div>
