...
export function CommentSection({
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        ...
        if (isMounted) {
          toast.error(error instanceof Error ? error.message : 'Không th? t?i b?nh lu?n');
        }
      } finally {
        if (isMounted) {
          setIsLoading(false);
  ...
  const handleAddComment = async (content: string) => {
    ...
    if (!currentUser) {
      toast.error('B?n c?n ðãng nh?p ð? b?nh lu?n');
      return;
    }
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Không th? g?i b?nh lu?n');
    } finally {
  ...
  return (
    <div className="px-4 py-3">
      {isLoading ? (
        <div className="py-4 text-sm text-gray-500">Ðang t?i b?nh lu?n...</div>
      ) : comments.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-8">
          ...
          <h3 className="mb-1 text-[17px] font-semibold text-gray-800">Chýa có b?nh lu?n nào</h3>
          <p className="text-[15px] text-gray-600">H?y là ngý?i ð?u tiên b?nh lu?n.</p>
        </div>
      ) : (
      ...
      <div className={isSubmitting ? 'pointer-events-none opacity-70' : ''}>
        <CommentInput
          ...
          placeholder={`B?nh lu?n dý?i tên ${authService.getCurrentUser()?.fullName || 'b?n'}`}
        />
      </div>
    </div>
  );
