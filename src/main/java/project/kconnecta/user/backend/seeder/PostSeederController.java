...
public class PostSeederController {
    ...
    private record Category(String name, String[] templates) {}

    private static final Category[] CATEGORIES = {
        new Category("Công nghệ", new String[]{
        ...
        new Category("Ẩm thực", new String[]{
            "Vừa thử quán %s mới, ngon không tưởng! Địa chỉ để trong comment #amthuc",
            ...
            "Món %s này khó làm nhưng ăn vào là quên hết mệt 🍜 #amthuc",
        ...
        new Category("Du lịch", new String[]{
            ...
            "Đang lên kế hoạch khám phá %s, ai có kinh nghiệm chia sẻ mình không? #dulich"
        }),
        new Category("Thể thao", new String[]{
        ...
        new Category("Giải trí", new String[]{
        ...
        new Category("Sức khỏe", new String[]{
            ...
            "%s: hiểu đúng để không bị mislead bởi thông tin sai #suckhoe"
        }),
        new Category("Giáo dục", new String[]{
        ...
        new Category("Thời trang", new String[]{
        ...
        new Category("Âm nhạc", new String[]{
        ...
        new Category("Gia đình", new String[]{
            ...
            "Làm gì khi có mâu thuẫn %s giữa các thành viên gia đình? #giadinh",
            "Kỳ nghỉ gia đình ở %s - kỷ niệm đáng nhớ nhất năm #giadinh",
            "%s là điều ý nghĩa nhất mình học được từ ông bà #giadinh",
            "Cân bằng giữa công việc và thời gian %s - bí quyết của mình #giadinh",
            "Ảnh gia đình chụp hôm nay - không cần filter vẫn đẹp 📸 #giadinh"
        })
    };

    private static final String[] NOUNS = {
        "smartphone", "laptop", "ứng dụng mới", "thiết bị", "phần mềm",
        "framework", "database", "cloud service", "kỹ thuật mới", "sản phẩm",
        "giải pháp", "phương pháp", "chủ đề hot", "xu hướng", "nội dung hay"
    };

    /**
     * POST /api/internal/seed/posts?force=true
     * force=true  → xóa posts cũ và seed lại
     * force=false → bỏ qua nếu đã có posts (default)
     */
    @PostMapping("/posts")
    @Transactional
    public ResponseEntity<Map<String, Object>> seedPosts(
        ...
        if (existing > 0 && !force) {
            return ResponseEntity.ok(Map.of(
                "skipped", true,
                "reason", "Already has " + existing + " posts. Use ?force=true to re-seed.",
        ...
        if (users.isEmpty()) {
            return ResponseEntity.badRequest().body(Map.of(
                "error", "No users found in database. Create users first."
            ));
        }

        if (force && existing > 0) {
            // TRUNCATE CASCADE xóa posts và tất cả bảng con liên quan (FK)
            jdbcTemplate.execute("TRUNCATE TABLE public.posts CASCADE");
            log.info("PostSeeder: truncated posts (and dependents) — was {} rows.", existing);
        }
        ...
        for (Category category : CATEGORIES) {
            for (int i = 0; i < POSTS_PER_CATEGORY; i++) {
                ...
                long minutesAgo = random.nextLong(30L * 24 * 60); // within last 30 days
                LocalDateTime publishedAt = now.minusMinutes(minutesAgo);

                Post post = Post.builder()
                    .author(author)
                    .content(content)
                    .privacy(PRIVACIES[random.nextInt(PRIVACIES.length)])
