...
function StatCard({ icon, label, value, sub, color }: { icon: React.ReactNode; label: string; value: string | number; sub?: string; color: string }) {
  return (
    <div className="bg-card rounded-xl border border-border p-4 flex items-start gap-3 shadow-sm dark:shadow-none hover:shadow-md transition-shadow">
...
function Toggle({ checked, onChange, disabled }: { checked: boolean; onChange: (v: boolean) => void; disabled?: boolean }) {
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${checked ? 'bg-violet-600' : 'bg-muted'} ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
    >
      <span className={`inline-block h-4 w-4 transform rounded-full bg-card shadow transition-transform ${checked ? 'translate-x-6' : 'translate-x-1'}`} />
    </button>
  );
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-foreground">AI Moderation Dashboard</h2>
            <p className="text-xs text-muted-foreground">Giám sát và kiểm soát nội dung tự động bằng AI</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full ${cfg.enabled ? 'bg-green-100 text-green-700' : 'bg-background text-muted-foreground'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-card rounded-xl border border-border shadow-sm dark:shadow-none overflow-hidden">
          <div className="px-5 py-4 border-b border-border bg-background flex items-center gap-2">
            ...
            <h3 className="font-semibold text-foreground text-sm">Cấu hình AI</h3>
          </div>
          <div className="p-5 space-y-5">
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="font-semibold text-foreground text-sm">AI Enabled</p>
                <p className="text-xs text-muted-foreground">Bật/tắt toàn bộ hệ thống AI Moderation</p>
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <p className="font-semibold text-foreground text-sm">Sensitivity</p>
              ...
              <p className="text-xs text-muted-foreground mt-1">{sensitivityInfo.desc}</p>
            </div>

            {/* Detect toggles */}
            <div>
              <p className="font-semibold text-foreground text-sm mb-3">Phát hiện</p>
            ...
            {/* Auto actions */}
            <div>
              <p className="font-semibold text-foreground text-sm mb-3">Hành động tự động</p>
              <div className="space-y-2.5">
                {([
        ...
        <div className="bg-card rounded-xl border border-border shadow-sm dark:shadow-none overflow-hidden">
          <div className="px-5 py-4 border-b border-border bg-background flex items-center gap-2">
            ...
            <h3 className="font-semibold text-foreground text-sm">AI Test Panel</h3>
          </div>
          <div className="p-5 space-y-4">
            <textarea
              ...
              rows={4}
              className="w-full border border-border rounded-lg px-3 py-2.5 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-violet-400 transition-shadow placeholder:text-muted-foreground"
            />
            <button
            ...
            {testResult && (
              <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
                {/* Overall */}
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold text-foreground">Overall Risk Score</span>
                  ...
                  {testResult.categories.map(cat => (
                    <div key={cat.label}>
                      <div className="flex items-center justify-between mb-1">
                        <span className={`text-xs font-medium flex items-center gap-1 ${cat.triggered ? 'text-red-600' : 'text-muted-foreground'}`}>
                ...
                {/* Predicted action */}
                <div className="mt-3 rounded-lg border border-border bg-background px-4 py-3 flex items-center justify-between">
                  <span className="text-sm text-muted-foreground font-medium">Hành động dự kiến</span>
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm dark:shadow-none overflow-hidden">
        <div className="px-5 py-4 border-b border-border bg-background flex items-center gap-2">
          ...
          <h3 className="font-semibold text-foreground text-sm">Score Range → Action Rules</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-background/60">
                <th className="text-left px-5 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Score Range</th>
                <th className="text-left px-5 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Action</th>
                <th className="text-left px-5 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Trạng thái</th>
                <th className="text-left px-5 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Mô tả</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {[
                ...
                <tr key={row.range} className="hover:bg-muted/50 transition-colors">
                  <td className="px-5 py-3.5 font-mono font-semibold text-foreground">{row.range}</td>
                  <td className="px-5 py-3.5">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${row.color}`}>{row.action}</span>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className={`flex items-center gap-1.5 text-xs font-medium ${row.active ? 'text-green-600' : 'text-muted-foreground'}`}>
                      <span className={`w-2 h-2 rounded-full ${row.active ? 'bg-green-500' : 'bg-muted'}`} />
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm dark:shadow-none overflow-hidden">
        <div className="px-5 py-4 border-b border-border bg-background flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-foreground text-sm">Recent Moderation Activity</h3>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-background/60">
                <th className="text-left px-5 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Thời gian</th>
                <th className="text-left px-5 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Loại</th>
                <th className="text-left px-5 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Nội dung</th>
                <th className="text-left px-5 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">AI Score</th>
                <th className="text-left px-5 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {MOCK_ACTIVITY.map(item => (
                <tr key={item.id} className="hover:bg-muted/50 transition-colors">
                  <td className="px-5 py-3.5 text-muted-foreground text-xs whitespace-nowrap">{item.time}</td>
                  <td className="px-5 py-3.5">
                  ...
                  <td className="px-5 py-3.5 text-muted-foreground max-w-[200px] truncate">{item.content}</td>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2">
