...
function formatGroupLabel(
  ...
  return formatVi(vi.birthdays.andOthers, {
...
function BirthdayFriendCard({ friend, showAge = true, onSendWish, onOpenCustomWish }: BirthdayFriendCardProps) {  const quickWishes = useMemo(
  ...
  const birthDateLabel = formatVi(vi.birthdays.birthDate, {
    day: birthDate.getDate(),
    month: birthDate.getMonth() + 1,
  });
  const timingLabel = friend.isToday
    ? vi.birthdays.todayLabel
    : formatVi(vi.birthdays.daysUntil, { count: friend.daysUntil });
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-muted-foreground">
              ...
              {showAge ? ` · ${formatVi(vi.birthdays.age, { age: friend.age })}` : ''}
            </p>
          </div>
        </div>
        <Link
...
export function BirthdayPage() {  const [searchQuery, setSearchQuery] = useState('');
  ...
  const handleQuickWish = async (friend: BirthdayFriend, message: string) => {
    try {
      ...
      toast.error((err as Error).message || vi.birthdays.sendError);
    }
  };
  ...
  if (error) {
    return (
      <div className="py-16 text-center text-muted-foreground">
        <p className="text-lg font-medium">{vi.birthdays.loadError}</p>
    ...
    <div className="flex h-full min-h-0 w-full flex-col">
      <div className="relative mb-6 max-w-xl shrink-0">
        ...
        <input
          ...
          placeholder={vi.birthdays.searchPlaceholder}
      ...
      <div className="min-h-0 flex-1 space-y-6 overflow-y-auto pr-1 sidebar-scrollbar">
          {showTodaySection && (
            <section className={PANEL_CLASS}>
              <div className="mb-4 flex items-center gap-2">
                ...
                <h2 className="text-xl font-bold text-foreground">{vi.birthdays.today}</h2>
              </div>
              {today.length > 0 ? (
                ...
                <p className="text-sm text-muted-foreground">{vi.birthdays.noToday}</p>
              )}
            </section>
          )}
          ...
          {showMonthSection && (
            <section className={PANEL_CLASS}>
              <h2 className="mb-4 text-xl font-bold text-foreground">{vi.birthdays.byMonth}</h2>
      ...
      {isSearching && !hasSearchResults && (
        <div className="py-16 text-center text-muted-foreground">
          <p className="text-lg font-medium">{vi.birthdays.noSearchResults}</p>
          <p className="mt-1 text-sm">{vi.birthdays.noSearchResultsHint}</p>
        </div>
      )}
      </div>
