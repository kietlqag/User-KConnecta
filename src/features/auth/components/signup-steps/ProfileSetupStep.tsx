...
export function ProfileSetupStep({
  ...
  return (
    <div>
      ...
      <button onClick={onBack} className="flex items-center gap-2 text-muted-foreground hover:text-foreground dark:hover:text-gray-100 mb-6 transition-colors">
        <ArrowLeft className="w-4 h-4" />
        <span className="text-sm font-medium">Quay lại</span>
      </button>

      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-foreground mb-2">Thiết lập hồ sơ</h2>
      ...
      {isGoogleSignup && email && (
        <div className="mb-6 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-center dark:border-emerald-900/50 dark:bg-emerald-950/30">
          <p className="text-sm text-muted-foreground">Tiếp tục với Google</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              onChange={handleChange}
              className={`w-full pl-12 pr-4 py-3 border-2 rounded-xl transition-all focus:outline-none ${ errors.dateOfBirth ? 'border-red-500 focus:border-red-500 focus:ring-2 focus:ring-red-200' : 'border-border focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200' }`}
            />
          </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <label
                key={label}
                className={`relative flex items-center justify-center p-3 border-2 rounded-xl cursor-pointer transition-all ${ profileData.gender === label ? 'border-emerald-500 bg-emerald-50' : 'border-border hover:border-gray-400' }`}
              >
                <input
                  type="radio"
                  name="gender"
                  ...
                  className="sr-only"
                />
                <span className={`text-sm font-medium inline-flex items-center gap-1.5 ${profileData.gender === label ? 'text-emerald-700' : 'text-foreground'}`}>
                  {icon}
                  {label}
                </span>
              </label>
        ...
        <div ref={locationPickerRef} className="relative">
          <label className="block text-sm font-medium text-foreground mb-2">Vị trí (Tùy chọn)</label>
          <div className="relative">
            <button
              type="button"
              onClick={() => setIsLocationPickerOpen((prev) => !prev)}
              className="relative w-full pl-12 pr-10 py-3 border-2 border-border rounded-xl text-left text-foreground hover:border-gray-400 transition-all focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200"
            >
              <MapPin size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground" />
              <span className={locationDisplay ? 'text-foreground' : 'text-muted-foreground'}>{locationDisplay || 'Chọn vị trí'}</span>
            ...
            {locationDisplay && (
              <button
                ...
                className="absolute right-11 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-muted-foreground"
          ...
          {isLocationPickerOpen && (
            <div className={`absolute z-20 w-full bg-card border border-border rounded-xl shadow-lg overflow-hidden max-h-[min(22rem,calc(100vh-14rem))] ${openUpward ? 'bottom-full mb-2' : 'top-full mt-2'}`}>
              <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-background">
                {locationStep === 'ward' ? (
                  <button type="button" onClick={() => setLocationStep('province')} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
                    <ChevronLeft size={16} />
                    Tỉnh/Thành phố
                  </button>
                ) : (
              ...
              <div className="px-3 py-3 border-b border-border space-y-2">
                <input
                  ...
                  className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500"
                />
                <button
                  type="button"
              ...
              <div className="overflow-y-auto max-h-[min(18rem,calc(100vh-18rem))]">
                {locationLoading ? (
                  <p className="px-4 py-3 text-sm text-muted-foreground">Đang tải dữ liệu...</p>
                ) : locationError ? (
                  ...
                  filteredProvinces.length > 0 ? (
                    filteredProvinces.map((province) => (
                      <button
                        ...
                        className="w-full px-4 py-3 text-left text-sm text-foreground hover:bg-emerald-50 hover:text-emerald-700 transition-colors"
                      >
                        {province.name}
                      </button>
                    ))
                  ) : (
                    <p className="px-4 py-3 text-sm text-muted-foreground">Không tìm thấy tỉnh/thành phố</p>
                  )
                  ...
                  <p className="px-4 py-3 text-sm text-muted-foreground">Không có dữ liệu xã/phường/đặc khu</p>
                )}
              </div>
            </div>
          )}
        ...
        <div>
          <label className="block text-sm font-medium text-foreground mb-2">Giới thiệu bản thân (Tùy chọn)</label>
          <textarea
            name="bio"
            ...
            rows={3}
            className="w-full px-4 py-3 border-2 border-border rounded-xl transition-all focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 resize-none"
            maxLength={200}
          />
          <p className="mt-1.5 text-xs text-muted-foreground text-right">{profileData.bio.length}/200</p>
        </div>
