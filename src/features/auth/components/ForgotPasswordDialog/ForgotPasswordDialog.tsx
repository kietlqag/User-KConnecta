...
export function ForgotPasswordDialog({ open, onClose }: ForgotPasswordDialogProps) {
  ...
  const inputClass = 'w-full px-4 py-3 border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm';
        ...
        <div className="px-6 py-6">
          ...
          {step === 'email' && (
            <form onSubmit={handleEmailSubmit} className="space-y-4">
              <div className="flex flex-col items-center mb-4">
                ...
                <p className="text-sm text-muted-foreground text-center">Nhập email để nhận mã xác thực đặt lại mật khẩu</p>
              </div>
              <div>
                <input
                  type="email"
          ...
          {step === 'reset' && (
            <form onSubmit={handleResetSubmit} className="space-y-4">
              <div className="flex flex-col items-center mb-4">
                ...
                <p className="text-sm text-muted-foreground text-center">Tạo mật khẩu mới cho tài khoản của bạn</p>
              </div>
              <div>
                <input
                  type="password"
              ...
              <div>
                <input
                  ...
                  className={`${inputClass} ${ isResetConfirmMatched ? 'border-green-500 focus:ring-green-500' : isResetConfirmMismatched ? 'border-red-500 focus:ring-red-500' : '' }`}
                />
              ...
              <div className="bg-background rounded-xl p-3 space-y-1.5">
                <p className="text-xs font-medium text-foreground">Mật khẩu phải có:</p>
                <p className="text-xs text-muted-foreground">{hasMinLength ? '✓' : '•'} Ít nhất 8 ký tự</p>
                <p className="text-xs text-muted-foreground">{hasUpperAndLower ? '✓' : '•'} Chữ hoa và chữ thường</p>
                <p className="text-xs text-muted-foreground">{hasNumber ? '✓' : '•'} Ít nhất một số</p>
              </div>
          ...
          {step === 'success' && (
            <div className="flex flex-col items-center text-center space-y-4 py-2">
              ...
              <div>
                <h3 className="text-xl font-bold text-foreground mb-1">Thành công!</h3>
