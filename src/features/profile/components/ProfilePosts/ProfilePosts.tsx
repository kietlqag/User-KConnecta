...
function PostSkeleton() {
  return (
    <div className="bg-card rounded-lg shadow mb-4 animate-pulse">
      <div className="p-4">
...
export function ProfilePosts({
  ...
  if (loading) {
    return (
      <div>
        <div className="bg-card rounded-2xl border border-border shadow-sm dark:shadow-none overflow-hidden mb-4">
  ...
  return (
    <div>
          ...
          {TAB_CONFIG.map(({ key, label, icon: Icon }) => {
            ...
            return (
              <button
                ...
                className={`flex items-center justify-center gap-1.5 px-2 py-3 text-sm font-medium transition-colors sm:gap-2 sm:px-4 ${ isActive ? 'text-primary dark:text-primary border-b-4 border-primary dark:border-primary bg-primary/5 dark:bg-primary/10' : 'text-muted-foreground hover:bg-muted' }`}
              >
      ...
      {filteredPosts.length === 0 ? (
        <div className="bg-card rounded-2xl border border-border shadow-sm dark:shadow-none p-12 flex flex-col items-center text-center">
        ...
        <div className="space-y-4">
          {filteredPosts.map((post) => (
            <Post key={post.id} {...post} onDelete={onDeletePost} onPostUpdated={onPostUpdated} />
          ))}
