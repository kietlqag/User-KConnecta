...
export function ProfileAboutPage() {
  ...
  return (
    <div className="mx-auto max-w-[1100px] px-4 py-6">
      <div className="flex min-h-[500px] flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-sm dark:shadow-none md:flex-row">
        <div className="w-full flex-shrink-0 border-b border-border pt-4 md:w-[260px] md:border-b-0 md:border-r">
          <div className="mb-3 flex items-center justify-between px-4">
            ...
            {isOwnProfile && (
              <button
                type="button"
                onClick={onEditClick}
                className="rounded-full p-2 text-muted-foreground transition-colors hover:bg-muted dark:text-muted-foreground"
          ...
          <nav className="space-y-1 px-3 pb-4">
            {ABOUT_TABS.map((tab) => (
              <button
                ...
                className={`w-full rounded-lg px-3 py-2.5 text-left text-[15px] font-medium transition-colors ${ activeTab === tab.id ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400' : 'text-muted-foreground hover:bg-muted dark:text-muted-foreground/60' }`}
              >
                {tab.label}
              </button>
            ))}
        ...
        <div className="flex-1 p-6">
          {currentRows.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-muted-foreground">{EMPTY_TAB_MESSAGES[activeTab]}</p>
            </div>
          ) : (
