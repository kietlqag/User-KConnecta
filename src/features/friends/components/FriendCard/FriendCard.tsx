      ...
      >
        ...
        {showRemove && onRemoveSuggestion && (
          <button
            ...
            className="absolute right-2 top-2 rounded-full bg-white/95 p-1.5 shadow-md transition-colors hover:bg-gray-100 dark:bg-gray-700/95 dark:hover:bg-gray-600"
            aria-label="Bỏ gợi ý"
          >
          ...
          {friend.isFriend ? (
            ...
            onAddFriend && (
              <button
                onClick={handleAdd}
                disabled={loading !== null}
                className="flex h-10 w-full cursor-pointer items-center justify-center gap-2 rounded-md bg-emerald-600 px-4 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {loading === 'add' ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
