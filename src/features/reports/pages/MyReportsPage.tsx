...
const STATUS_CONFIG: Record<ReportStatus, { label: string; className: string; icon: React.ReactNode }> = {
  ...
  REVIEWED: { label: 'Đang xem xét', className: 'bg-blue-100 text-blue-700',   icon: <Eye className="w-3 h-3" /> },
  RESOLVED: { label: 'Đã xử lý',    className: 'bg-green-100 text-green-700',  icon: <CheckCircle className="w-3 h-3" /> },
};

const SEVERITY_CONFIG: Record<string, { label: string; className: string }> = {
  NONE:   { label: 'Không vi phạm', className: 'bg-gray-100 text-gray-600' },
      ...
      {!loading && reports.length > 0 && (
        <div className="space-y-3">
          {reports.map((report) => {
            ...
            return (
              <div key={report.id} className="rounded-xl border border-gray-200 bg-white p-4 space-y-3">
                ...
                {report.aiAnalysis && (
                  <div className="text-sm bg-gray-50 rounded-lg p-3 border border-gray-100">
                    <span className="font-medium text-gray-500 text-xs uppercase tracking-wide">Phân tích AI</span>
                    <p className="mt-1 text-gray-700">{report.aiAnalysis}</p>
                  </div>
                )}
              </div>
            );
