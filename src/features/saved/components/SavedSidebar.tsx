...
export const SavedSidebar = ({
  ...
  return (
    <aside className="w-[360px] h-[calc(100vh-56px)] bg-card border-r border-border flex flex-col fixed left-0 top-14 z-20">
      <div className="p-4">
        <h1 className="text-2xl font-bold text-foreground">{vi.saved.title}</h1>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2">
        ...
        >
          ...
          <span className="font-semibold text-[15px]">{vi.saved.allItems}</span>
        </button>

        {collections.length > 0 && (
          <>
            <div className="mt-4 px-3 mb-2">
              <h3 className="text-[17px] font-bold text-foreground">{vi.saved.myCollections}</h3>
            </div>
        ...
        >
          ...
          <span className="font-semibold text-[15px]">{vi.saved.createCollection}</span>
        </button>
      </div>
    </aside>
  );
