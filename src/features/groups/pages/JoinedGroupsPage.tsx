            ...
            {loadingJoined ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className="bg-white rounded-lg border border-gray-200 h-[200px] animate-pulse" />
                ))}
              </div>
            ) : joinedGroups.length === 0 ? (
              <p className="text-gray-500 text-[15px]">Bạn chưa tham gia nhóm nào.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {joinedGroups.map(group => (
                  <div
                    key={group.id}
                    className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden flex flex-col h-[200px] hover:shadow-md transition-shadow"
                  >
                    {/* Top Section */}
                    <div className="p-4 flex-1 flex gap-3 border-b border-gray-100">
                      {group.icon ? (
                        <img
                          src={group.icon}
                          alt={group.name}
                          className="w-[84px] h-[84px] rounded-xl object-cover shrink-0 border border-gray-100"
                        />
                      ) : (
                        <div className="w-[84px] h-[84px] rounded-xl bg-gray-200 shrink-0 flex items-center justify-center text-gray-400 text-2xl font-bold">
                          {group.name.charAt(0)}
                        </div>
                      )}
