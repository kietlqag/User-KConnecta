...
export const ManagedGroupsPage = () => {
  ...
  const { data: managedGroups = [], isLoading: loadingManaged } = useManagedGroups(sortKey);
  const { data: sidebarManagedGroups = [] } = useManagedGroups();
  const { data: joinedGroups = [] } = useJoinedGroups();

  return (
    <GroupsHubLayout
      joinedGroups={joinedGroups}
      managedGroups={sidebarManagedGroups}
      ...
      <div className="flex h-[calc(100vh-88px)] flex-col">
        ...
        <div className="sidebar-scrollbar min-h-0 flex-1 overflow-y-auto pr-1">
          {loadingManaged ? (
            ...
            <p className="text-[15px] text-muted-foreground">Bạn chưa quản lý nhóm nào.</p>
          ) : (
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
