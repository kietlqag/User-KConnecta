...
export function GroupRequestsTab({ groupId, onApproveSuccess }: GroupRequestsTabProps) {
  ...
  if (isLoading) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 flex flex-col items-center justify-center min-h-[300px]">
        ...
        <p className="text-gray-500 text-sm font-medium">Đang tải danh sách yêu cầu...</p>
      </div>
    );
  }
  ...
  if (requests.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center flex flex-col items-center justify-center min-h-[350px]">
        ...
        <h3 className="text-lg font-bold text-gray-900 mb-2">Không có yêu cầu tham gia nào</h3>
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="divide-y divide-gray-100">
        {requests.map((req) => {
          ...
          return (
            <div key={req.id} className="p-4 sm:p-5 flex items-center justify-between gap-4 hover:bg-gray-50/50 transition-colors">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <h4 className="font-semibold text-gray-900 text-sm sm:text-[15px] truncate hover:underline cursor-pointer">
                    {req.fullName}
                  </h4>
                  <p className="text-gray-500 text-xs mt-0.5">Yêu cầu tham gia nhóm</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-xs sm:text-sm flex items-center gap-1 transition-all active:scale-[0.98] disabled:opacity-50 cursor-pointer"
                >
                ...
                <button
                  ...
                  disabled={approveMutation.isPending || rejectMutation.isPending}
                  className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-xs sm:text-sm flex items-center gap-1 transition-all active:scale-[0.98] shadow-sm hover:shadow disabled:opacity-50 cursor-pointer"
                >
