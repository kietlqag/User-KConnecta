...
export const InviteFriendsModal: React.FC<InviteFriendsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      {/* Backdrop */}
      <div className="absolute inset-0 bg-card/80 backdrop-blur-sm" onClick={onClose} />
      
      {/* Modal Content */}
      <div className="relative bg-card w-full max-w-[680px] rounded-xl shadow-2xl border border-border overflow-hidden flex flex-col h-[540px]">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <div className="w-8" /> {/* Spacer */}
          <h2 className="text-[20px] font-bold text-foreground">Mời bạn bè tham gia nhóm này</h2>
          <button 
            onClick={onClose}
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-[60%] flex flex-col border-r border-border">
            ...
            <div className="flex-1 overflow-y-auto px-2">
              <div className="px-3 mb-2">
                <span className="text-[15px] font-semibold text-foreground">Gợi ý</span>
              </div>
              
              {isLoading ? (
                <div className="p-4 text-center text-muted-foreground text-sm">Đang tải bạn bè...</div>
              ) : filteredFriends.length === 0 ? (
                <div className="p-4 text-center text-muted-foreground text-sm">Không tìm thấy bạn bè nào</div>
              ) : (
                <div className="space-y-1">
                  {filteredFriends.map((friend) => (
                    ...
                    >
                      ...
                      />
                      <span className="flex-1 text-[15px] font-medium text-foreground">{friend.name}</span>
                      <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${ selectedUserIds.includes(friend.userId) ? 'bg-emerald-600 border-emerald-600' : 'border-border group-hover:border-gray-400' }`}>
          ...
          <div className="w-[40%] bg-background/50 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {selectedFriends.map((friend) => (
                <div key={friend.userId} className="flex items-center gap-3">
                  ...
                  />
                  <span className="flex-1 text-[14px] font-medium text-foreground truncate">{friend.name}</span>
                  <button 
                    onClick={() => toggleUserSelection(friend.userId)}
        ...
        {/* Footer */}
        <div className="p-4 border-t border-border flex items-center justify-end gap-3 bg-card">
          <button 
            onClick={onClose}
          ...
          <button 
            ...
            className="px-8 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-muted disabled:text-muted-foreground text-white rounded-lg font-semibold transition-all text-[15px] shadow-sm dark:shadow-none flex items-center gap-2"
          >
