...
const destinationOptions: LiveDestinationOption[] = [
  {
    id: 'profile',
    label: 'Ðãng lên trang cá nhân',
    description: 'Ngý?i t? ch?c - Trang cá nhân c?a b?n',
    icon: '??',
  },
  {
    id: 'page',
    label: 'Ðãng lên trang b?n qu?n l?',
    description: 'Chia s? ð?n trang c?a b?n',
    icon: '??',
  },
  {
    id: 'group',
    label: 'Ðãng trong nhóm',
    ...
    <div className="w-80 bg-white border-r border-gray-200 p-4">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <h3 className="font-semibold text-sm">{currentUser?.fullName || 'Ngý?i dùng'}</h3>
          <p className="text-xs text-gray-500">Ngý?i t? ch?c - Trang cá nhân c?a b?n</p>
        </div>
      </div>

      <div className="mb-6">
        <label className="text-sm font-medium text-gray-700 mb-2 block">Ch?n nõi ðãng</label>
      ...
      <div className="mt-8 p-4 bg-blue-50 rounded-lg">
        ...
        <ul className="space-y-2 text-xs text-blue-800">
          ...
          <li>- Ð?m b?o ánh sáng ð?y ð?</li>
          <li>- Chu?n b? n?i dung trý?c khi b?t ð?u</li>
          <li>- Týõng tác v?i ngý?i xem</li>
        </ul>
      </div>
    </div>
  );
