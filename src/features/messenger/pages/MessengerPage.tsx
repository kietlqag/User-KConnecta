  ...
  try {
    ...
    const fallbackLabel =
      payload?.kind === 'completed'
        ? mediaType === 'video'
          ? 'Cu?c g?i video hoàn thành'
          : 'Cu?c g?i tho?i hoàn thành'
        : mediaType === 'video'
          ? 'Ð? b? l? cu?c g?i video'
          : 'Ð? b? l? cu?c g?i tho?i';
  ...
  return (
    ...
    >
      ...
      <button className="w-full flex items-center justify-between rounded-lg px-2 py-3 hover:bg-gray-50 transition-colors text-left">
        <span className="text-[15px] font-medium text-gray-800">File phýõng ti?n & file</span>
        <ChevronDown className="w-5 h-5 text-gray-500" />
      </button>
      <button className="w-full flex items-center justify-between rounded-lg px-2 py-3 hover:bg-gray-50 transition-colors text-left">
        <span className="text-[15px] font-medium text-gray-800">Quy?n riêng tý và h? tr?</span>
        <ChevronDown className="w-5 h-5 text-gray-500" />
      </button>
    </aside>
  );
            ...
            <div className="p-4 border-b border-gray-200">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <h1 className="text-[1.75rem] font-semibold tracking-tight">Ðo?n chat</h1>
                  <span
                    className={`w-2 h-2 rounded-full ${connected ? 'bg-green-500' : 'bg-gray-300'}`}
                    title={connected ? 'Ð? k?t n?i realtime' : 'Chýa k?t n?i'}
                  />
                </div>
                <div className="flex items-center gap-2">
                  <button
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {loadingConversations ? (
                ...
                <div className="text-center py-8 text-sm">
                  <p className="text-red-500 mb-2">Không th? t?i danh sách b?n bè</p>
          ...
          {activeChatUser ? (
            ...
            <div className="flex-1 flex items-center justify-center bg-white rounded-xl border border-gray-200">
              <div className="text-center px-6">
                ...
                <p className="text-gray-500 text-sm">Ch?n m?t cu?c tr? chuy?n ð? b?t ð?u nh?n tin</p>
              </div>
            </div>
          )}
