...
import { Link2, MessageCircle, Images, ArrowLeft, Search, Globe, Lock, Smile, ChevronDown, Newspaper, Users } from 'lucide-react';
...
export function AlbumShareModal({
  ...
  const buildMessengerShareContent = () =>
    `${ALBUM_SHARE_PREFIX}${JSON.stringify({
      id: albumId,
      title: albumTitle,
  ...
  const handleSendToFriend = async (userId: string, userName: string) => {
    ...
    try {
      ...
      void albumService.sendToUser(albumId, userId).catch(() => {});
  ...
  const handleShareToFeed = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Không thể chia sẻ album');
    } finally {
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText(`${window.location.origin}/albums/${albumId}`);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) handleClose(); }}>
      <DialogContent className="sm:max-w-[500px] p-0 overflow-visible bg-white dark:bg-gray-800 rounded-2xl border-none shadow-2xl">
        <DialogDescription className="sr-only">Chia sẻ album lên bảng tin hoặc gửi cho bạn bè</DialogDescription>
        {showFriendPicker ? (
          <>
            ...
            <div className="p-4 flex flex-col gap-4">
              ...
              <div className="relative">
                <textarea
                  ...
                  placeholder="Hãy nói gì đó về album này..."
                  rows={3}
                  maxLength={1000}
              ...
              <div className="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                {coverUrl ? (
                  <img src={coverUrl} alt={albumTitle} className="h-28 w-full object-cover" />
                ) : (
                  <div className="flex h-24 w-full items-center justify-center bg-gradient-to-br from-violet-100 to-blue-100 dark:from-gray-700 dark:to-gray-800">
                    <Images className="h-8 w-8 text-violet-500/70" aria-hidden />
                  </div>
                )}
                <div className="p-3">
                  <p className="truncate text-sm font-semibold text-gray-900 dark:text-gray-100">{albumTitle}</p>
                  <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                    {mediaCount} ảnh/video{ownerName ? ` · ${ownerName}` : ''}
                  </p>
                </div>
              </div>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    navigate('/stories/create', {
                      state: {
                        sharedImageUrl: coverUrl || null,
                        sharedText: albumTitle,
                      },
                    });
                  }}
