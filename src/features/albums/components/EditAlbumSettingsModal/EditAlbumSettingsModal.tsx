...
export function EditAlbumSettingsModal({ album, media, isOpen, onClose }: EditAlbumSettingsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="flex max-h-[min(90vh,720px)] w-full max-w-lg flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-800">
        <div className="flex shrink-0 items-center justify-between border-b border-gray-200 px-4 py-3 dark:border-gray-700">
          <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">Cài đặt album</h3>
          <button
            type="button"
            onClick={onClose}
            className="rounded-full p-2 text-gray-500 hover:bg-muted dark:text-gray-400"
            aria-label="Đóng"
          >
            <X className="h-5 w-5" />
          </button>
        ...
        <div className="scrollbar-thin min-h-0 flex-1 space-y-5 overflow-y-auto p-4">
          <div>
            <label htmlFor="album-settings-title" className="mb-1.5 block text-sm font-semibold text-gray-800 dark:text-gray-200">
              Tên album
            </label>
            <input
          ...
          <div>
            <label htmlFor="album-settings-description" className="mb-1.5 block text-sm font-semibold text-gray-800 dark:text-gray-200">
              Mô tả
            </label>
            <textarea
          ...
          {!isGroupAlbum ? (
            <div>
              <p className="mb-2 text-sm font-semibold text-gray-800 dark:text-gray-200">Quyền riêng tư</p>
            ...
            <div className="rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm text-gray-600 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
              Album nhóm luôn hiển thị công khai với thành viên nhóm.
            </div>
          )}

          <div>
            <p className="mb-2 text-sm font-semibold text-gray-800 dark:text-gray-200">Ảnh bìa</p>
            {selectedCover ? (
              ...
              <p className="mb-3 text-sm text-gray-500">Chưa có ảnh trong album để chọn làm bìa.</p>
            )}
            {media.length > 0 && (
        ...
        <div className="flex shrink-0 gap-2 border-t border-gray-200 px-4 py-4 dark:border-gray-700">
          <button
            type="button"
            onClick={onClose}
            disabled={isSaving}
            className="flex-1 rounded-lg bg-gray-100 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-200 disabled:opacity-60 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-700"
          >
            Hủy
          </button>
          <button
